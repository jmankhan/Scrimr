...
const RegistrationForm = () => {
  ...
  const handleSubmit = async () => {
    setIsLoading(true);
    try {
      const result = await auth.register(data.email, data.summonerName, data.password);
      navigate('/');
    } catch(err) {
      ...
      <Grid.Column style={{ maxWidth: 450 }}>
        ...
        <Form size='large' error={message}>
          <Segment stacked>
            ...
            <Form.Field fluid>
              <Input name='summonerName' fluid label='Summoner Name' onChange={handleInput} />
            </Form.Field>
            <Form.Field fluid>
              <Input
                name='password'
            ...
            <Button primary fluid size='large' onClick={handleSubmit} loading={isLoading}>
              Register
            </Button>
            <Message
              error
