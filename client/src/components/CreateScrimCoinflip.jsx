...
import { Button, Grid, Dropdown, Header, Segment } from "semantic-ui-react";
...
const CreateScrimCoinflip = (props) => {
  ...
  const handleFlip = () => {
    ...
    const nextCaptain =
      captains[
        (captains.findIndex((captain) => captain.id === caller.id) + 1) %
          captains.length
      ];
  ...
  const handleSubmit = () => {
    ...
    setResultMessage(
      `${
        captains.find((c) => c.id === draftFirst).summoner.name
      } will draft first.\n` +
        `${captains.find((c) => c.id === sideFirst).summoner.name} will be on ${
          sideSelection === 0 ? "Team 1" : "Team 2"
        }.`
    );

    props.onChange({
      coinflipWinner: winner.id,
      draftFirst,
      sideFirst,
      sideSelection,
    });
  ...
  const getSideFirstName = () => {
    ...
    const sideFirst = reward === 1 ? winner : loser;
    return props.members.find((member) => member.id === sideFirst).summoner
      .name;
  };
    ...
    <Grid centered columns={3}>
      <Grid.Row>
        <Grid.Column>
          {caller && (
            <Segment>
              <Header className="dropdownLabel">
                {caller.summoner.name} chooses:
              </Header>
              <Dropdown
                options={[
                  { text: "Heads", value: 0 },
      ...
      <Grid.Row>
        <Grid.Column>
          <Button primary fluid disabled={hasFlipped} onClick={handleFlip}>
            Flip
          </Button>
        </Grid.Column>
      </Grid.Row>
          ...
          <Grid.Row>
            <Grid.Column>
              <Segment>
                ...
                >
                  {props.members.find((m) => m.id === winner).summoner.name}{" "}
                  chooses to:
                </Header>
                <Dropdown
                  options={[
                    {
                      text: "Draft first player",
                      value: 0,
                    },
                    { text: "Choose map side", value: 1 },
                  ]}
                  defaultValue={reward}
                  simple
          ...
          <Grid.Row>
            <Grid.Column>
              <Button
                ...
                fluid
                disabled={hasPickedReward}
                onClick={() => setHasPickedReward(true)}
              >
                Pick Reward
              </Button>
            </Grid.Column>
